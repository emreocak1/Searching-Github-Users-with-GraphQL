...
export const calculateMostForkRepos = (repositories:Repository[]):{repo:string;count:number}[] => {
...
export const calculatePopularLanguages = (repositories: Repository[]):{language:string;count:number}[] => {
  ...
  return Object.entries(languageMap).sort(([, a], [, b]) => b - a).slice(0, 5).map(([language,count])=>({language,count}))
}